...
class Model
{
    ...
    public static function init($id = 0, $softClass = false)
    {
		...
		if($id !== 0)
		{
            if(is_numeric($id))
            {
                $result=ModelQuery::table($ins1::$table, static::class)->select()->where([(static::class)::getPrimaryKey(), '=', $id])->limit(0,1)->run();
	...
	public function save()
	{
        ...
        if($this->getId() === 0)
        {
            $id=ModelQuery::table(static::getTableName(), static::class)->insert($insertCol)->run();
            $this->setId($id);
        }
        else
        {
            ModelQuery::table(static::getTableName(), static::class)->update($insertCol)->where([(static::class)::getPrimaryKey(), '=', $this->getId()])->run();
        }
	}
